const prompt1 = "Do you head 'left' or 'right'?";
const prompt2 =
...
const prompt3 =
  "You follow the cat to a colony of cats, nestled in a fort of warm blankets and subsisting off of inexplicably warm soup. They are content if you 'stay', but you wonder if you should 'alert the world' to this magical safe haven.";
...
const prompt9 =
  "You come across a snoring dragon. 'Past the dragon', you see a shiny chest of treasure. Another path would lead you 'away from the dragon' altogether. Which do you take?";
const prompt10 =
...
if (answer === "left") {
  let answer = window.prompt(prompt2);
  if (answer === "follow") {
    let answer = window.prompt(prompt3);
    if (answer === "stay") {
      let answer = window.alert(prompt4);
    } else if (answer === "spread the word") {
      let answer = window.alert(prompt5);
    }
  } else if (answer === "continue") {
    let answer = window.prompt(prompt6);
    if (answer === "ladder") {
      let answer = window.alert(prompt7);
    } else if (answer === "staircase") {
      let answer = window.alert(prompt8);
    }
  }
} else if (answer === "right") {
  let answer = window.prompt(prompt9);
  if (answer === "past the dragon") {
    let answer = window.prompt(prompt10);
    if (answer === "stay") {
      let answer = window.alert(prompt11);
    } else if (answer === "run") {
      let answer = window.alert(prompt12);
    }
  } else if (answer === "away from the dragon") {
    let answer = window.prompt(prompt13);
    if (answer === "draw") {
      let answer = window.alert(prompt14);
    } else if (answer === "pick") {
      let answer = window.alert(prompt15);
    }
  }
}
